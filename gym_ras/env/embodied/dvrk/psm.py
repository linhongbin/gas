...
class SinglePSM():
    ...
    def __init__(self,
        ...
        self._world2base=getT([0,0,0], [0,0,world2base_yaw], rot_type="euler", euler_convension="xyz", euler_Degrees=True)
    ...
    def move_gripper_init_pose(self):
        pos_rel = self._gripper_pose_rng.uniform(self._init_pose_low_gripper,self._init_pose_high_gripper)
    ...
    def _set_action(self, action: np.ndarray):
        ...
        action[:3] *=self._max_step_pos  # position, limit maximum change in position
        pose_world = self.tip_pose
        pose_world[:3, 3] =  np.clip(pose_world[:3, 3] + action[:3], self.workspace_limit[:,0], self.workspace_limit[:,1])
        rot = M2Euler(pose_world[:3, :3],convension="xyz", degrees=False)
        if self._action_mode == 'yaw':
            action[3] *= np.deg2rad(self._max_step_rot)  # yaw, limit maximum change in rotation
            rot[2] = wrapAngle(rot[2] + action[3],degrees=False, angle_range=180)  # only change yaw
        ...
        if action[4] < 0:
            ...
            self._psm.jaw.move_jp(np.deg2rad(self._open_gripper_deg)).wait()  # open jaw angle; can tune
