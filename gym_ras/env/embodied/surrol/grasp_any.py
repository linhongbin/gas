...
class GraspAny(GraspAnyBase):
    WORKSPACE_LIMITS1 = (
        (0.50, 0.60), (-0.05-0.02, 0.05+0.02), (0.675+0.0088, 0.745))
    ...
    def _env_setup(self):
        asset_path = Path(__file__).resolve(
        ).parent.parent.parent.parent / "asset" / "urdf"
        file_dir = {
        ...
        file_dir = {k: [asset_path / k / _v for _v in v]
        ...
        _low = self._stuff_scaling_low if str(_dir).find(
            "needle") < 0 else self._needle_scaling_low
        _high = self._stuff_scaling_high if str(_dir).find(
            "needle") < 0 else self._needle_scaling_high
        scaling = self._stuff_urdf_rng.uniform(_low,  _high)
        # scaling = 1

        # print("urdf scaling:",scaling, "dir: ",_dir)

        super()._env_setup(stuff_path=str(_dir), scaling=scaling)
