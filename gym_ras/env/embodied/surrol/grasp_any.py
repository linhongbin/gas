...
class GraspAny(GraspAnyBase):
    WORKSPACE_LIMITS1 = ((0.50, 0.60), (-0.05-0.02, 0.05+0.02), (0.675+0.0088, 0.745))
    def __init__(self, 
    ...
    def _env_setup(self):
        asset_path = Path(__file__).resolve().parent.parent.parent.parent / "asset" / "urdf"
        file_dir = {
        ...
        _low = self._stuff_scaling_low if str(_dir).find("needle")<0 else  self._needle_scaling_low
        _high = self._stuff_scaling_high if str(_dir).find("needle")<0 else  self._needle_scaling_high
