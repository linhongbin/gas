...
class DSA(BaseWrapper):
    ...
    def __init__(self, 
        ...
        self._image_encode_id = {"stuff":encoding_id_stuff, "psm1": encoding_id_psm1, "zoom_box": encoding_id_zoom_box}
    ...
    def dsa_process(self, img,pfix):
        ...
        if self._encode_type == "IROS2023":
            zoom_x_min, zoom_x_max, zoom_y_min, zoom_y_max = zoom_box_x[0], zoom_box_x[1], zoom_box_y[0], zoom_box_y[1]
            ...
            layer1 = self._segment(img["depth"+pfix], gripper_mask | stuff_mask)
            zoom_box_length = self._zoom_box_fix_length_ratio * min(zoom_mask.shape[0],zoom_mask.shape[1])
            ...
            zoom_x_min, zoom_x_max, zoom_y_min, zoom_y_max = self._get_zoom_coordinate(zoom_box_x, zoom_box_y, zoom_box_length, zoom_mask.shape)
            ...
            gripper_ecode = self._segment(self._scale_arr(img["depth"+pfix], 0, 255, 0, 127), gripper_mask)
            stuff_ecode = self._segment(self._scale_arr(img["depth"+pfix], 0, 255, 128, 255), stuff_mask)
            encode_mat = np.uint8(gripper_ecode + stuff_ecode)
            layer3 = self._zoom_legal(encode_mat, zoom_x_min, zoom_x_max,zoom_y_min, zoom_y_max)
            ...
            for k,v in img["mask"+pfix].items():
                if k in self._dsa_key:
                    ...
                    if self._encode_type == 'general_simple3':
                        ...
                        _box_r = list(_box_r)
                        _box_c = list(_box_c)
                        _box_r[0], _box_r[1] = self._get_legal_min_max_continuous(_box_r[0],_box_r[1], _l, zoom_mask.shape[0])
                        _box_c[0], _box_c[1] = self._get_legal_min_max_continuous(_box_c[0],_box_c[1],_l, zoom_mask.shape[1])
                    if k == "stuff":
                        stuff_x = _box_r
                        stuff_y = _box_c
                    layer1[_box_r[0]:_box_r[1]+1, _box_c[0]:_box_c[1]+1] += self._image_encode_id[k]
                    _union_mask = (_union_mask|v) if _union_mask is not None else v
            
            self._is_out_dsa_zoom([zoom_x_min,zoom_x_max], [zoom_y_min, zoom_y_max], stuff_x, stuff_y)

            if self._encode_type == 'general_simple':
                layer1[zoom_x_min:zoom_x_max+1, zoom_y_min:zoom_y_max+1] += self._image_encode_id["zoom_box"]
            elif self._encode_type in ['general_simple2','general_simple3',]:
                layer1[zoom_x_min:zoom_x_max+1, zoom_y_min:zoom_y_max+1] = self._image_encode_id["zoom_box"]
            if "depth" in img:
                depth_seg_mat = self._segment(img["depth"+pfix],_union_mask)
                layer2 = self._zoom_legal(depth_seg_mat, zoom_x_min, zoom_x_max,zoom_y_min, zoom_y_max)
            else:
            ...
            for k,v in img["mask"+pfix].items():
                if k in self._dsa_key:
                    _mask_mat[v] = self._image_encode_id[k]
            layer3 = self._zoom_legal(_mask_mat, zoom_x_min, zoom_x_max,zoom_y_min, zoom_y_max)
            im_pre = np.stack([layer1,layer2, layer3], axis=2)
            img_dsa = im_pre

        if self._timestep_prv != self.env.timestep:
            self._zoom_coordinate_prv = (zoom_x_min, zoom_x_max, zoom_y_min, zoom_y_max)
    ...
    def _is_out_dsa_zoom(self, zoom_x, zoom_y, stuff_x, stuff_y, margin=0.2):
        ...
        self._out_zoom =  (stuff_x[0]>zoom_x[1]-m1) or (stuff_x[1]<zoom_x[0]+m1) or (stuff_y[0]>zoom_y[1]-m2) or (stuff_y[1]<zoom_y[0]+m2)
    ...
    def _get_zoom_coordinate(self, zoom_box_x, zoom_box_y, zoom_box_length, zoom_mask_shape):        
        if self._zoom_movement_type == "continuous":
            zoom_x_min, zoom_x_max = self._get_legal_min_max_continuous(zoom_box_x[0],zoom_box_x[1], zoom_box_length, zoom_mask_shape[0])
            zoom_y_min, zoom_y_max = self._get_legal_min_max_continuous(zoom_box_y[0],zoom_box_y[1], zoom_box_length, zoom_mask_shape[1])
        elif self._zoom_movement_type == "discrete":
            if self._zoom_coordinate_prv is None:
                zoom_x_min, zoom_x_max = self._get_legal_min_max_continuous(zoom_box_x[0],zoom_box_x[1], zoom_box_length, zoom_mask_shape[0])
                zoom_y_min, zoom_y_max = self._get_legal_min_max_continuous(zoom_box_y[0],zoom_box_y[1], zoom_box_length, zoom_mask_shape[1])
            else:
                zoom_x_min, zoom_x_max = self._get_legal_min_max_discrete(zoom_box_x[0],zoom_box_x[1],self._zoom_coordinate_prv[0],self._zoom_coordinate_prv[1],zoom_box_length, zoom_mask_shape[0])
                zoom_y_min, zoom_y_max = self._get_legal_min_max_discrete(zoom_box_y[0],zoom_box_y[1], self._zoom_coordinate_prv[2],self._zoom_coordinate_prv[3], zoom_box_length, zoom_mask_shape[1])            
        else:
