...
class ImageNoise(BaseWrapper):
    ...
    def _post_process(self, img):
        ...
        if self._cutout_all_amount_range[0]<1:
            img = self._cutout_protocol(img, mask_min=self._cutout_all_amount_range[0], mask_max=self._cutout_all_amount_range[1], only_depth=False)
        if self._cutout_depth_amount_range[0]<1:
            img = self._cutout_protocol(img, mask_min=self._cutout_depth_amount_range[0], mask_max=self._cutout_depth_amount_range[1], only_depth=True)
    ...
    def _cutout_protocol(self, img, mask_min, mask_max=0.9, only_depth=False):
        ...
        for loop in range(self._max_loop):
            ...
            for k,v in _num.items():
                for _ in range(v):
                    _img = self._cutout(_img, cutout_type=k,only_depth=only_depth)
