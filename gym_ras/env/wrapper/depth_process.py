...
class DepthProcess(BaseWrapper):
    ...
    def _no_depth_guess_process(self, imgs,):
        ...
        for target, source in self.unwrapped.nodepth_guess_map.items():
            ...
            uncert = self.unwrapped.nodepth_guess_uncertainty[target] * self._uncert_scale
            uncert_pix = scale_arr(uncert, 0, self.unwrapped.depth_remap_range[0][1]-self.unwrapped.depth_remap_range[0][0], 0, 255) // 2
            if not self._eval:
                _guess_val += np.random.uniform(-uncert_pix,uncert_pix)
                
            rand_mat = np.clip(np.random.uniform(_guess_val-uncert_pix,_guess_val+uncert_pix,size=imgs["depth"].shape), 0, 255)
            # print(rand_mat)
