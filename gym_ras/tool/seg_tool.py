...
class ColorSegmentor():
    ...
    def set_param(self,
        ...
        arr =np.array([self.param["hsv_h"],self.param["hsv_s"],self.param["hsv_v"], self.param["hsv_h2"],])
        ...
        for v in self.pipeline:
            if v == "hsv":
                ...
                for h in [self.param["hsv_h"],self.param["hsv_h2"],]:
                    hsv_range = np.array([h,self.param["hsv_s"],self.param["hsv_v"], ])
                    img_hsv=cv2.cvtColor(rgb_arr,cv2.COLOR_RGB2HSV) 
                    img_target=cv2.inRange(img_hsv,tuple(hsv_range[:,0].tolist()),tuple(hsv_range[:,1].tolist()))
                    _m = img_target>0 
                ...
                _range = np.int32((min(_shape[0],_shape[1]) * self.param["region_mask_size"]) // 2)
                _row = (np.int32(np.clip(row_c-_range, 0, _shape[0])), np.int32(np.clip(row_c+_range, 0, _shape[0])))
                _col = (np.int32(np.clip(column_c-_range, 0, _shape[1])), np.int32(np.clip(column_c+_range, 0, _shape[1])))
