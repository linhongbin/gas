        ...
        while True:
            ...
            for k, v in imgs.items():
                ...
                if k.find("image")>=0:
                    ...
                    imgs_dict[k] = np.stack([v]*3, axis=2) if len(v.shape)==2 else v
                elif k.find("dsa")>=0:
                    imgs_dict[k] = v
                    for i in range(v.shape[2]):
                        imgs_dict[k+"@"+str(i)] = np.stack([v[:,:,i]]*3, axis=2)
                elif k.find("mask")>=0:
                    if len(v)==0:
                        break
                    ...
                    for _key, _value in dict(sorted(v.items())).items():
                        # print(_value.shape)
                        _mask = _value if _mask is None else np.logical_or(_mask, _value)
                    ...
                    c = 0.2
                    b = 1
                    imgs_dict[k+"@enhance"] = cv2.addWeighted(_background, c, _mask_rgb, 1-c, b)
                # elif k == "mask":
                #     _img = v
                elif k.find("obs")>=0:
                    # print(cv2.resize(v,
            ...
            if "rgb" in imgs_dict and "depth" in imgs_dict:
                c = 0.4
                b = 1
                imgs_dict["rgbd@enhance"] = cv2.addWeighted(imgs_dict["rgb"], c, imgs_dict["depth"], 1-c, b)
            ...
            for j in range(id_shape.shape[0]):
                ...
                for i in range(id_shape.shape[1]):
                    ...
                    cv2.putText(_v, str(k), (20,20 ), cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255, 204, 0), 1)
                    # print(_v.shape)
                    frame = _v if frame is None else np.concatenate((frame, _v), axis=1)
                frames.append(frame)
            ...
            _t = 0 if self._update_hz<0 else np.int32((1/self._update_hz)*1000)
